...
class MainWindowLayout(QMainWindow):
    ...
    def __init__(self, app, *args):
        ...
        self.useCell = QRadioButton('Set cell')
        ...
        self.useRmerge = QRadioButton('Use R_merge 1st and 2nd pass')
        self.useRmerge.setCheckable(True)
        self.Rmerge_cut = QDoubleSpinBox()
        self.Rmerge_cut.setDecimals(2)
        self.Rmerge_cut.setSingleStep(0.1)
        self.useCcHalf = QRadioButton('Use C 1/2 1st and 2nd pass')
        self.useCcHalf.setCheckable(True)
        self.CcHalf_cut = QDoubleSpinBox()
        self.CcHalf_cut.setDecimals(2)
        self.CcHalf_cut.setSingleStep(0.1)
        self.useIoverSig = QRadioButton(u'Use I/\u03C3(I) 1st and 2nd pass')
        self.useIoverSig.setCheckable(True)
        self.IoverSig_cut = QDoubleSpinBox()
        self.IoverSig_cut.setDecimals(2)
        self.IoverSig_cut.setSingleStep(0.1)
        # Default values and ranges
        self.CcHalf_cut.setMinimum(-2.0)
        self.CcHalf_cut.setMaximum(1.0)
        self.IoverSig_cut.setMinimum(1.0)
        self.IoverSig_cut.setMaximum(1000)
        self.Rmerge_cut.setValue(99.9)
        self.CcHalf_cut.setValue(-1.0)
        self.IoverSig_cut.setValue(0.0)
        ...
        self.first_image.setMaximum(99999)
        self.last_image = QSpinBox()    
        self.last_image.setMinimum(1)
        self.last_image.setMaximum(99999)
        ...
        self.cellGB.layout().addWidget(self.useCell,1,0)
        ...
        self.cutsGB = QGroupBox('Cutoff parameters in highest resolution shell')
        self.cutsGB.setLayout(QGridLayout())
        self.cutsGB.layout().addWidget(self.useRmerge,0,0,1,2)
        self.cutsGB.layout().addWidget(self.Rmerge_cut,1,0)
        self.cutsGB.layout().addWidget(self.useCcHalf,0,2,1,2)
        self.cutsGB.layout().addWidget(self.CcHalf_cut,1,2)
        ...
        self.connect(self.Rmerge_cut, SIGNAL('valueChanged(double)'), self.useRmergeCheck)
        self.connect(self.CcHalf_cut, SIGNAL('valueChanged(double)'), self.useCcHalfCheck)
        self.connect(self.IoverSig_cut, SIGNAL('valueChanged(double)'), self.useIoverSigCheck)
        self.connect(self.imgName, SIGNAL('textChanged(QString)'), self.useImageSpecCheck)
        self.connect(self.first_image, SIGNAL('valueChanged(int)'), self.useImageSpecCheck)
        self.connect(self.last_image, SIGNAL('valueChanged(int)'), self.useImageSpecCheck)
